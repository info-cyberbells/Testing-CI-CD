...
const GoLive = () => {
          ...
          setTimeout(() => {
            ...
            if (isRecording && recognitionRef.current) {
              ...
              try {
                ...
                lastSpeechTimestampRef.current = Date.now() - 5000;

                // if (broadcastIdRef.current) {

                //   fetch('https://churchtranslator.com/speech/translate_realtime', {
                ...
                //   }).catch(e => console.error('Error sending keepalive:', e));
    ...
    try {
      const response = await fetch('https://churchtranslator.com/speech/translate_realtime', {
        ...
        body: JSON.stringify({
          ...
          broadcastId: broadcastIdRef.current, // Include broadcast ID
          slidingWindowInfo: {
            segmentId: id,
            startWord: startIdx,
            endWord: endIdx,
  ...
  const initializeSpeechRecognition = () => {
    ...
    //   if (isRecording) {
    //     const delay = Math.min(500 * Math.pow(2, retriesRef.current), 3000);
    //     retriesRef.current++;
    ...
    try {
      ...
      if (data.liveSermonCount > 0) {
        ...
        if (churchLiveSermons.length > 0) {
          const liveSermon = churchLiveSermons[0];
  ...
  const initializeTranslationStream = () => {
    ...
    translationSourceRef.current = new EventSource(
      `https://churchtranslator.com/speech/stream_translation/${selectedLanguage}?client_id=${clientIdRef.current}&role=broadcaster&broadcast_id=${broadcastIdRef.current}&instance_id=${instanceIdRef.current}`
    );
    ...
    translationSourceRef.current.onmessage = (event) => {
      try {
        ...
        // Check instance ID to ensure this message is for our instance
  ...
  // Fetch church languages immediately on mount
  useEffect(() => {
    const fetchChurchLanguages = async () => {
      try {
        ...
        const churchId = urlParams.get("churchId");
        if (!churchId) {
          console.error("No church ID found in URL");
        ...
        const transformedOptions = {
          male: goLiveLanguages.male.map(lang => ({
            value: lang.id,
            label: `${lang.language.length > 15 ? lang.language.substring(0, 15) + '...' : lang.language} (M)` // Shorter for mobile
          })),
          female: goLiveLanguages.female.map(lang => ({
            value: lang.id,
            label: `${lang.language.length > 15 ? lang.language.substring(0, 15) + '...' : lang.language} (F)` // Shorter for mobile
          }))
        };
  ...
  const startRecording = async () => {
    ...
    try {
      ...
      const userId = queryParamsObj.get("userId");
      const churchId = queryParamsObj.get("churchId");
      ...
      // Start the stream with the backend
      ...
      if (error.message.includes('No API Configrations Found with this Church') || error.message.includes('No API Keys Configured')) {
        displayError = 'No API Configrations Found with this Church';
        ...
        displayError = 'No API Configrations Found with this Church'; // Default for stream start failures
      }
  ...
  const stopRecording = async () => {
    try {
      ...
      // Stop the stream with the broadcast ID
  ...
  // Handle visibility change to re-acquire wake lock if the page becomes visible again
  useEffect(() => {
            ...
            <FormControl fullWidth>
              <StyledSelect
                ...
                MenuProps={{
                  PaperProps: {
                    style: {
                      maxHeight: 250, // Limit height for mobile
                      width: 'auto',
                    },
                  },
                  MenuListProps: {
